...
export function useEthPrices(): EthPrices | undefined {
  const [prices, setPrices] = useState<{ datas: EthPrices}>()
  ...
  useEffect(() => {
    async function fetch() {
      const { data, error } = await fetchEthPrices(formattedBlocks as [number, number, number], client)
      if (error || blockError) {
        setError(true)
      } else if (data) {
        setPrices({
  ...
  }, [error, prices, formattedBlocks, blockError, client, indexedPrices])
